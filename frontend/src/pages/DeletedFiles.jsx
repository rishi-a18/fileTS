...
import { Clock, AlertTriangle, CheckCircle, FileText, Trash2, RotateCcw } from 'lucide-react';
...
const DeletedFiles = () => {
    ...
    const fetchDeletedFiles = async () => {
        try {
            const response = await api.get('/file/deleted');
        ...
        <div className="space-y-6 animate-fade-in-up">
            <div className="flex justify-between items-center">
                <div className="flex items-center space-x-3">
                    ...
                    <h2 className="text-3xl font-bold text-slate-800 tracking-tight">Deleted Files</h2>
                </div>
            </div>
                ...
                <div className="overflow-x-auto">
                    <table className="min-w-full whitespace-nowrap">
                        <thead className="bg-slate-50 border-b border-slate-100">
                            <tr>
                                {['File Name', 'Section', 'Priority', 'Status', 'Upload Date', 'Deletion Remarks', 'Actions'].map((header) => (
                            ...
                            {files.length > 0 ? (
                                files.map((file) => (
                                    <tr key={file.id} className="hover:bg-slate-50/80 transition-colors duration-150">
                                        ...
                                        <td className="px-6 py-4 text-sm text-slate-700 italic border-l border-slate-100 bg-red-50/30">
                                            {file.deletion_remarks || <span className="text-slate-400">No remarks</span>}
                                        </td>
